...
const CustomUpload = () => {
  ...
  const [selectedFile, setSelectedFile] = useState(null); // Selected file for preview
  ...
  const handleFileUpload = (e) => {
    ...
    setUploadedFiles((prev) => [...prev, ...filePreviews]); // Append files to the existing list
  };
  ...
  // Confirm submission of the selected file
  const confirmFile = () => {
    if (selectedFile) {
      alert(`File ${selectedFile.file.name} submitted successfully!`);
            ...
            {uploadedFiles.length > 0 ? (
              uploadedFiles.map((file) => (
                ...
                >
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation(); // Prevent triggering preview on remove
                      removeFile(file.id);
                    }}
